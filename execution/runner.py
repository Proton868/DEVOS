...
class ExecutionLayer:
    ...
    async def run(
        ...
        *,
        ...
        # Build environment — never inherit host secrets
        from execution.governed_exec import scrub_env
        extra = {}
        if env_vars:
            extra.update(env_vars)
        if secrets:
            for k, v in secrets.items():
                extra[f"SECRET_{k.upper()}"] = str(v)
        env = scrub_env(extra=extra, allow_secret_prefix=True)
...
async def run_command_in_project(
    ...
    *,
    ...
    """Run a command scoped to a user project root under mandatory isolation.
    ...
    Prefer argv= for simple commands; shell_command form uses governed shell
    mode when metacharacters require it (still isolated for untrusted).

    Returns dict including ok, exit_code, stdout, stderr, command, duration_ms,
    status, mode, and isolation evidence fields for audit/acceptance.
    """
    from execution.files import PROJECTS_DIR, PathViolation, _safe_scope_segment
    from execution.governed_exec import run_governed, scrub_output
    from execution.isolation import normalize_policy

    if argv is None and (not command or not str(command).strip()):
        return {
            "ok": False,
            "exit_code": 2,
            "stdout": "",
    ...
    # Resolve and re-check after mkdir (symlink escape)
    ...
    try:
        ...
        return {
            ...
            "exit_code": 2,
            "stdout": "",
            "stderr": "project root escapes projects directory after resolve",
            "command": cmd or str(argv),
            "status": "failed",
            "isolation_evidence": {
                "trust_level": normalize_policy(policy),
                "policy_decision": "denied",
                "failure_reason": "symlink/path escape",
            },
        }
    ...
    try:
        ...
        d = result.to_dict()
        d["stdout"] = scrub_output(d.get("stdout") or "")
        d["stderr"] = scrub_output(d.get("stderr") or "")
        return d
    except Exception as e:
