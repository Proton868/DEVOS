...
## 15. Human IDE path vs agent mission path

### Human IDE path

A human user may directly:
...
- inspect runtime
- use IDE assistance
- perform ordinary project operations

These are **legitimate human-controlled** DevOS operations. They are **not** automatically agent missions.

### Agent mission path

Consequential autonomous/agent work should follow:
...
**Human direct control and agent-controlled execution are complementary paths, but they must remain attributable and distinguishable.**

Human IDE activity must never be represented as specialist-agent accomplishment merely because it happened inside DevOS.

---

## 16. Website generation and scaffold policy

Current implementation reality:

- An A2A/specialist website path exists (**PARTIALLY IMPLEMENTED**).
- Website artifact validation exists (**PARTIALLY IMPLEMENTED**).
- A Python `scaffold_website_artifacts()` fallback may exist behind an environment flag.
- Scaffold output is **not** equivalent to specialist-agent execution.
- Scaffold fallback is **DEPRECATED / EMERGENCY-ONLY / NON-PRODUCTION SUCCESS PATH**.
- Live specialist website execution on deployed systems is **TESTED BUT NOT LIVE-PROVEN** unless proven on that host.

An artifact generated by `scaffold_website_artifacts()` does **not** constitute evidence that the specialist website agent successfully completed the mission.

---

## 17. MCP boundary (documentation)

**Native internal capability (preferred for internal resources):**

```text
Nuha → Agent → Native DevOS Capability → Resource
```
...
Filesystem MCP or similar presets that wrap internal resources must not be treated as the preferred architecture when an equivalent native DevOS capability exists.

---

## 18. Anti-drift rule

Future features must be evaluated against this vision.

A feature is **architectural drift** if it causes DevOS to increasingly become merely:

- a chatbot
- a prompt-to-app generator
- an AI IDE only
- an automation platform only
- an MCP wrapper
- an uncontrolled autonomous agent

The product must continue moving toward:

**Unified development environment + delegated agents + governed execution + evidence + verification + continuous improvement.**

---

## 19. Lifecycle status snapshot
