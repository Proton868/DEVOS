    ...
    async def dispatch(self, request, requester_identity, provider=None, model=None, on_step=None, agent=None):
        ...
        try:
            ...
            if "task" in sig.parameters:
                out = await WorkerRuntime().run(requester_identity=requester_identity, provider=provider, model=model, on_step=on_step, task=request)
                if len(out) == 3: state, identity, result = out
                else:
                    state, identity = out
                    result = TaskResult(task_id=request.task_id, execution_id=request.execution_id,
                        worker_slug=request.worker_slug, status=TaskStatus.SUCCEEDED if getattr(state,"succeeded",False) else TaskStatus.FAILED,
                        output=getattr(state,"final_answer",None), agent_id=agent.agent_id, attempt=request.attempt)
            else:
                state, identity = await WorkerRuntime().run(request.worker_slug, request.objective, requester_identity, provider=provider, model=model, on_step=on_step)
                decision = str(getattr(state, "decision", ""))
                status = TaskStatus.SUCCEEDED if "complete" in decision else TaskStatus.FAILED
                result = TaskResult(task_id=request.task_id, execution_id=request.execution_id, worker_slug=request.worker_slug,
                    status=status, output=getattr(state, "final_answer", None), agent_id=agent.agent_id, attempt=request.attempt,
                    parent_task_id=request.parent_task_id, root_loop_id=request.root_loop_id, loop_id=getattr(state,"id",None))
        except Exception as e:
            logger.exception("dispatch failed")
            result = TaskResult(task_id=request.task_id, execution_id=request.execution_id, worker_slug=request.worker_slug,
                status=TaskStatus.FAILED, errors=[str(e)], failure_reason=str(e), agent_id=agent.agent_id, attempt=request.attempt)
